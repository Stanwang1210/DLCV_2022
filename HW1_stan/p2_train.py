...
def main(args):
    ...
    model.train()
    Train_Loss, Train_IOU, Eval_Loss, Eval_IOU, = [], [], [], []
    for epoch in range(args.epoch):
        ...
        for data, label, file_name in p_bar:
            ...
            iou = mean_iou_score(pred, label.numpy())  *100
            ...
            scheduler.step
            optimizer.zero_grad()
            p_bar.set_description(f'Train Loss : {loss.item():.2f}, Train IOU : {iou:.2f}%')
            p_bar.refresh()
        Train_Loss.append(np.mean(Loss)) 
        Train_IOU.append(np.mean(IOU)) 
        with torch.no_grad():
            Loss = []
            IOU = []
            model.eval()
            for data, label, file_name in tqdm(valid_loader):
                ...
                Loss.append(loss.item())
                IOU.append(iou)
            logger.info(f'At Epoch {epoch}, Eval Loss : {np.mean(Loss):.2f}, Eval IOU : {np.mean(IOU):.2f}%')
            Eval_Loss.append(np.mean(Loss)) 
            Eval_IOU.append(np.mean(IOU))
...
if __name__ == '__main__':
    ...
    # model 
    parser.add_argument("--model_name_or_path", type=str, default='vgg16')
